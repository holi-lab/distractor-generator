"""
SFT training code for the pairwise ranker.

The SFT training set must include the following column:
"text": instruction prompt (question, answer, and distractors must be filled in) + reasoning + choice
...
train_df = pd.read_csv("pr_sft_train.csv") # SFT training set
training_data = Dataset.from_pandas(train_df[['text']]) # target column: 'text'
